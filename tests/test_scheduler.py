...
class TestSchedulerService:
    ...
    def test_init_with_daily_callback(self):
    ...
    def test_start_adds_refresh_job(self, mock_sched_cls):
    ...
    @patch("services.scheduler.BackgroundScheduler")
    def test_start_with_daily_callback_adds_two_jobs(self, mock_sched_cls):
        mock_sched = MagicMock()
        mock_sched_cls.return_value = mock_sched
    ...
    @patch("services.scheduler.BackgroundScheduler")
    def test_stop_shuts_down(self, mock_sched_cls):
        mock_sched = MagicMock()
        mock_sched_cls.return_value = mock_sched
    ...
    @patch("services.scheduler.BackgroundScheduler")
    def test_run_refresh_now_calls_callback(self, mock_sched_cls):
        svc = SchedulerService(self.refresh_cb)
        svc.run_refresh_now()
        self.refresh_cb.assert_called_once()

    @patch("services.scheduler.BackgroundScheduler")
    def test_refresh_job_has_correct_id(self, mock_sched_cls):
