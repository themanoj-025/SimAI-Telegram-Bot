...
class TestAICompareScraper:
    ...
    def test_compare_alias_resolution(self):
    ...
    def test_compare_model_with_extra_text(self):
...
class TestAIRoadmapScraper:
    ...
    def test_ai_engineer_roadmap(self):
...
class TestAILeaderboardScraper:
    ...
    @pytest.mark.asyncio
    async def test_leaderboard_returns_data(self):
        result = await self.scraper.get_leaderboard()
        assert len(result) > 0

    @pytest.mark.asyncio
    async def test_leaderboard_has_top_models(self):
        result = await self.scraper.get_leaderboard()
    ...
    @pytest.mark.asyncio
    async def test_leaderboard_with_filter(self):
