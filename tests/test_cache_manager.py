...
class TestCacheManagerInit:
    ...
    def test_creates_tables(self, cache):
...
class TestUpdateAndGetCache:
    ...
    def test_cache_not_expired_within_window(self, cache):
    ...
    def test_update_replaces_old_data(self, cache):
        cache.update_cache("cat", [{"v": 1}])
        cache.update_cache("cat", [{"v": 2}])
        result = cache.get_cached_data("cat")
        assert result == [{"v": 2}]

    def test_get_latest_cached_data(self, cache):
        cache.update_cache("cat", [{"v": 1}])
        time.sleep(0.01)
        cache.update_cache("cat", [{"v": 2}])
...
class TestDeduplication:
    ...
    def test_mark_as_seen_idempotent(self, cache):
        cache.mark_as_seen("cat", "item_1")
        cache.mark_as_seen("cat", "item_1")  # duplicate insert
        assert cache.is_duplicate("cat", "item_1") is True

    def test_clear_old_seen_items(self, cache):
    ...
    def test_clear_preserves_recent_items(self, cache):
...
class TestEdgeCases:
    ...
    def test_none_category(self, cache):
        # Should not raise
        result = cache.get_cached_data(None)
        assert result is None

    def test_special_characters_in_data(self, cache):
